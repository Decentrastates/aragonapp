...
export interface ISetupAppsBodyDialogExternalAddressProps {
    /**
     * ID of the DAO.
     */
    daoId: string;
...
export const SetupAppsBodyDialogExternalAddress: React.FC<ISetupAppsBodyDialogExternalAddressProps> = (props) => {
    const { daoId } = props;
    const { network } = daoUtils.parseDaoId(daoId);
    const { t } = useTranslations();

    const { setValue } = useFormContext<IAppsSetupBodyForm>();

    const {
        onChange: onReceiverChange,
        value,
        ...addressField
    } = useFormField<IAppsSetupBodyForm, 'address'>('address', {
