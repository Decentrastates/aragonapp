...
export const SwapInfo: React.FC<ISwapInfoProps> = () => {
    ...
    const useCustomTokenAData = useWatch<{ useCustomTokenA?: boolean }>({
        control,
        name: 'body.useCustomTokenA' as never,
    });
    ...
    const customTokenAData = useWatch<{ customTokenA?: string }>({
        control,
        name: 'body.customTokenA' as never,
    });
                ...
                {effectiveTokenA ? (
                    <p className="mt-2 text-sm text-green-700">
                        {useCustomTokenA && customTokenA
                            ? t('app.plugins.draw.createDrawForm.step4.swapRules.tokenAInfoWithCustomAddress', {
                                  tokenA: effectiveTokenA,
                              })
                            : t('app.plugins.draw.createDrawForm.step4.swapRules.tokenAInfoWithAddress', {
                                  tokenA: effectiveTokenA,
                              })}
                    </p>
                ) : (
